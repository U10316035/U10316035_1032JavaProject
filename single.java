...
public class single extends JFrame{
	...
	JButton start = new JButton("回到選單");
	...
	single(String userName,String userId,int timeSet){
		for(int j = 0;j <= 9;j++){
			checkQ[j] = false;
		}
		user = userName;
		...
		start.setBounds(450, 275, 125, 30);
		start.setFont(new Font("標楷體", Font.BOLD, 20));
		start.setVisible(false);
		panel.add(start);
		
		start.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e){
				sure.setTitle("u_sure?");
				sure.setSize(200, 100);
				sure.setLocationRelativeTo(null); // Center the frame
				sure.setDefaultCloseOperation(JFrame.DISPOSE_ON_CLOSE);
				...
				ok.setBounds(15, 20, 75, 25);
				ok.setFont(new Font("標楷體", Font.BOLD, 20));
				sure.add(ok);
				
				no.setBounds(110, 20, 75, 25);
				no.setFont(new Font("標楷體", Font.BOLD, 20));
				sure.add(no);
				...
				//關閉指令
				sure.addWindowListener(new WindowAdapter(){
		...
		/*class CloseHandler implements WindowListener{
				@Override
